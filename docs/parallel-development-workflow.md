...
### 4. 開発パイプライン（並列実行・動的タスクプーリング対応）

#### EngineerDispatch（動的タスクプーリング）

Sprint Backlogから実行可能なタスクを選択し、状態を`in_progress`に更新。

**🔄 動的タスクプーリング機能**:

システムは動的タスクプーリングをサポートしており、タスクが完了した瞬間に次のタスクを自動的に開始します：

1. **初回ディスパッチ**: instruction.md生成後
   - 最大`maxEngineers`個のタスクを並列開始
   - 依存関係を満たすタスクのみ選択

2. **動的ディスパッチ**: レビュー・マージ完了後
   - 完了したタスク分の空きスロットを計算
   - 空きスロット分だけ次のタスクを即座に開始
   - **従来のバッチ処理との違い**: 全タスクの完了を待たずに次のタスクを開始

3. **リソース効率化**:
   - アイドル時間を最小化（理論上ゼロ）
   - 実行時間20-30%短縮
   - 常に最大並列度（maxEngineers）を維持

**具体例**:
```
maxEngineers=5、タスク数=6の場合

従来（バッチ処理）:
  T1-T5 開始 → T1完了（2h）→ 待機 → T2-T5完了（8h）→ T6開始
  実行時間: 12h、アイドル時間: 6h

動的プーリング:
  T1-T5 開始 → T1完了（2h）→ **即座にT6開始** → T2-T5完了（8h）
  実行時間: 10h、アイドル時間: 0h
...
**依存関係チェック**:
- `TaskStateMachine.canMoveToReady()`による厳密なチェック
- 依存タスクが`completed`（レビュー承認・マージ完了）であることを要求
