 ...
 */
export async function backlogRefinementNode(state) {
    const { globalTasks, config } = state;
    const maxTurns = config.maxTurns || 50;
    console.log('🔄 BacklogRefinement: バックログのリファインメントを開始します...');
    // 既存タスクのチェック
    const pendingTasks = globalTasks.filter((task) => task.status === 'pending' && !task.sprint);
    ...
    // 更新されたタスクを適用
    const updatedGlobalTasks = applyRefinementUpdates(globalTasks, refinementData);
    // タスク分割提案を抽出
    const taskSplitSuggestions = refinementData.taskSplits || [];
    // タスク統合提案を抽出
    const taskMergeSuggestions = refinementData.taskMerges || [];
    // 更新されたタスクを保存
    ...
    if (refinementData.updatedTasks) {
        refinementData.updatedTasks.forEach((updatedTask) => {
 ...
 * バックログリファインメントプロンプトを構築
 */
function buildRefinementPrompt(tasks, sprintHistory) {
 ...
 */
function applyRefinementUpdates(globalTasks, refinementData) {
    const updatedTasksMap = new Map((refinementData.updatedTasks || []).map((t) => [t.id, t]));
    return globalTasks.map((task) => {
        ...
        return {
            ...
            estimatedHours: update.estimatedHours !== undefined
                ? update.estimatedHours
                : task.estimatedHours,
            dependencies: update.dependencies !== undefined
                ? update.dependencies
                : task.dependencies,
            businessValue: update.businessValue !== undefined
                ? update.businessValue
                : task.businessValue,
            technicalRisk: update.technicalRisk !== undefined
...
//# sourceMappingURL=BacklogRefinementNode.js.map