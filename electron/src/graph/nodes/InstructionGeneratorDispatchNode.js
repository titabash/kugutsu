 ...
 */
export async function instructionGeneratorDispatchNode(state) {
    ...
    console.log(`📤 ${tasksToDispatch.length}個のタスクを並列ディスパッチ ` +
 ...
 */
export function instructionGeneratorDispatchRouter(state) {
    if (!state.activeSprint?.id) {
    ...
    console.log(`➡️ ルーティング: instruction_generator (未生成: ${tasksNeedingInstruction.length}件)`);
    return 'generate';
}
//# sourceMappingURL=InstructionGeneratorDispatchNode.js.map