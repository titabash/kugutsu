 ...
 */
async function generateInstructionForTask(state, task, config) {
    ...
    if (state.storyMapping && state.designDocs) {
        // 高複雑度パス: 設計書を参照
        context = buildHighComplexityContext(state.storyMapping, state.designDocs, task);
    }
    else {
        // 低複雑度パス: ユーザーリクエストとタスク定義を参照
        context = buildLowComplexityContext(state.userRequest, task);
    }
...
出力形式: Markdown
  `.trim();
    ...
    if (!fileExists) {
        ...
        throw new Error(`instruction.md の作成に失敗しました (タスク: ${task.id})\n` +
            `ファイルパス: ${instructionPath}\n` +
            `詳細: ${errorMessage}`);
    }
 ...
 * 高複雑度パス用のコンテキスト構築
 */
function buildHighComplexityContext(storyMapping, designDocs, task) {
    return `
# ストーリーマッピング
 ...
 * 低複雑度パス用のコンテキスト構築
 */
function buildLowComplexityContext(userRequest, task) {
    return `
# ユーザーリクエスト
...
//# sourceMappingURL=InstructionGeneratorNode.js.map