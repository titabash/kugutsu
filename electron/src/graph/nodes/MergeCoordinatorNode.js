...
export async function mergeCoordinatorNode(state) {
    ...
    try {
        ...
        for (const task of reviewedTasks) {
            try {
                const review = await fileReader.readJSON(`.kugutsu/tasks/${task.id}/review.json`);
        ...
        for (const mergeTask of newMergeTasks) {
            try {
                ...
                try {
                    ...
                    await AIFileWriter.writeFile(provider, `.kugutsu/tasks/${mergeTask.taskId}/merge-result.json`, mergeResult, config.baseRepoPath);
                    ...
                    if (taskToUpdate) {
                        await AIFileWriter.updateTaskInTasksJson(provider, tasksPath || '.kugutsu/tasks.json', mergeTask.taskId, {
                            status: 'completed',
                    ...
                    if (errorOutput.includes('conflict') || errorOutput.includes('CONFLICT')) {
                        ...
                        await AIFileWriter.writeFile(provider, `.kugutsu/tasks/${mergeTask.taskId}/conflicts.json`, conflicts, config.baseRepoPath);
