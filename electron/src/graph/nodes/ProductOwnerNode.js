...
`;
        ...
        const requirementsResult = await RetryManager.executeWithRetry(async () => {
...
`;
        ...
        const taskGenerationResult = await RetryManager.executeWithRetry(async () => {
            const handler = new MessageHandler({
                maxTurns,
        ...
        const globalTasks = tasks.map((task) => {
            return {
                id: task.id,
                type: 'feature', // ProductOwner generates feature tasks
                title: task.title,
                description: task.description,
                priority: task.priority,
                dependencies: task.dependencies,
