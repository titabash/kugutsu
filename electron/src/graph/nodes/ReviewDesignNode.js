...
export async function reviewDesignNode(state) {
    ...
    // 設計書を読み込み
    const designDocsMarkdown = await loadDesignDocument(persistence, currentProjectId, 'design-docs.md');
    const wireframesMarkdown = await loadDesignDocument(persistence, currentProjectId, 'wireframes.md');
    const erDiagramMarkdown = await loadDesignDocument(persistence, currentProjectId, 'er-diagram.md');
    const apiSpecMarkdown = await loadDesignDocument(persistence, currentProjectId, 'api-spec.md');
    // 設計書が存在しない場合
 ...
 * レビューを実行
 */
async function executeReview(provider, reviewer, designDocs, storyMapping, cwd, maxTurns) {
...
function consolidateReviews(directorReview, productOwnerReview, techLeadReview) {
    ...
    // 総合評価
    const overallAssessment = buildOverallAssessment(allReviewers, criticalIssues, majorIssues);
    return {
        approved,
...
//# sourceMappingURL=ReviewDesignNode.js.map