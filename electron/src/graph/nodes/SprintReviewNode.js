...
## 完了タスク一覧
${JSON.stringify(completedTasks.map((t) => ({
            title: t.title,
 ...
 */
export function sprintReviewRouter(state) {
    // 全タスク完了チェック
    const allTasksCompleted = state.globalTasks.every((task) => task.status === 'completed' || task.status === 'failed');
