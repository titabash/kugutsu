...
2. **Design Docs確認**（必須・最優先）:
   - **.kugutsuディレクトリの確認**: プロジェクトルートの.kugutsuディレクトリ内にDesign Docsが存在するか確認
   - **Design Docs読み込み**: design-doc-*.mdファイルが存在する場合は必ず読み込んで内容を理解
   - **設計指針の把握**: Design Docsが存在する場合、サイトマップ、API設計、命名規則、共通仕様などチーム全体の設計指針を確認
   - **アーキテクチャ理解**: Design Docsが存在する場合、システム構成図、技術スタック、ディレクトリ構造などの設計情報を把握
   - **Design Docs不在時**: Design Docsが存在しない場合は、既存コードから設計パターンと規約を推測して実装
3. **プロジェクト設計の体系的理解**（最重要・必須）:
   - **アーキテクチャパターン分析**: 既存のアーキテクチャパターンを特定・理解
   - **ドメイン設計の把握**: ビジネスドメイン、エンティティ関係、ドメインルールの理解
   - **コーディング規約の確立**: プロジェクト全体の統一規約とパターンの把握
4. **要件分析**: タスクの要件を正確に把握し、期待される入出力を明確化
5. **技術調査とベストプラクティスの活用**（重要）:
   - **Web検索の積極活用**:
...
6. **タスクタイプ判定**: 機能実装か、ドキュメント・設定変更かを判定
7. **機能実装の場合**:
...
8. **ドキュメント・設定変更の場合**:
   - 直接実装を行う（テスト不要）
9. **最終確認**: すべてのテストが通ることを確認してからコミット
...
効率的で高品質なコードを心がけてください。`;
        ...
        try {
            ...
            for await (const message of query({
                ...
                if (message && typeof message === 'object' && 'type' in message) {
                    this.displayMessageActivity(message, output);
                }
            }
     ...
     * メッセージアクティビティを表示
     */
    displayMessageActivity(message, output) {
     ...
     * ツール実行の詳細を表示
     */
    displayToolExecutionDetails(toolName, toolInput, _toolId, toolExecutionId) {
        switch (toolName) {
            case 'Read':
     ...
     * ツール実行結果を表示
     */
    displayToolResult(result, _toolId) {
    ...
    buildTaskPrompt(task) {
        const instructionFile = task.instructionFile;
    ...
    async getChangedFiles(worktreePath) {
        try {
            ...
            return output
                ...
                .map((line) => line.substring(3)); // ステータス文字を除去
        }
        catch (error) {
...
//# sourceMappingURL=EngineerAI.js.map