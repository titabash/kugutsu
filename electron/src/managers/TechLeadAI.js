...
効率的で厳格なレビューを心がけ、プロジェクトの整合性を重視してください。`;
     ...
     * エンジニアAIの成果物をレビュー
     */
    async reviewEngineerWork(task, engineerResult) {
        ...
        try {
            ...
            for await (const message of query({
                ...
                if (message && typeof message === 'object' && 'type' in message) {
                    const reviewText = this.displayMessageActivity(message);
                    if (reviewText) {
     ...
     * ツール実行の詳細を表示
     */
    displayToolExecutionDetails(toolName, toolInput, executionId) {
        switch (toolName) {
            case 'Read':
     ...
     * レビュー用プロンプトを構築
     */
    buildReviewPrompt(task, engineerResult) {
        return `
🔍 テックリードによるコードレビュー開始
...
厳格かつ建設的なレビューを実施してください。品質向上のため遠慮なく指摘してください。`;
    ...
    parseReviewStatus(comments) {
        ...
        const hasArchitectureViolation = architectureViolations.some(violation => fullText.includes(violation));
     ...
     * MergeCoordinatorを設定
     */
    setMergeCoordinator(mergeCoordinator) {
        this.mergeCoordinator = mergeCoordinator;
    }
    /**
     * レビュー承認後の協調マージを実行
     */
    async performCoordinatedMerge(task, reviewResult) {
        ...
        // コンフリクト解消用のコールバック関数
        const conflictResolutionHandler = async (conflictTask, engineerId, existingEngineer) => {
        ...
        // 協調マージを実行
        const mergeResult = await this.mergeCoordinator.coordinatedMerge(task, conflictResolutionHandler);
...
//# sourceMappingURL=TechLeadAI.js.map