...
export class TaskConverter {
     ...
     * @returns State用のTask
     */
    static toStateTask(artifact, existingTasks = []) {
        // ステータス変換
        const status = this.convertArtifactStatusToStateStatus(artifact, existingTasks);
        // 日時変換: string → Date
     ...
     */
    static canMoveToReady(artifact, existingTasks) {
        // 依存関係がない場合は ready
     ...
     * @returns Task のステータス
     */
    static convertArtifactStatusToStateStatus(artifact, existingTasks) {
     ...
     */
    static convertStateStatusToArtifactStatus(status) {
        switch (status) {
            case 'pending':
                return 'pending';
            case 'in_progress':
