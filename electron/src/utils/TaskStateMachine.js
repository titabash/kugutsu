...
export class TaskStateMachine {
     ...
     */
    static validateTransition(task, newStatus) {
        if (!this.canTransition(task.status, newStatus)) {
            throw new InvalidStateTransitionError(`Invalid status transition: ${task.status} → ${newStatus} for task ${task.id}`);
        }
    }
    /**
     * 安全な状態遷移実行
     ...
     */
    static transition(task, newStatus, context) {
        ...
        switch (`${task.status}->${newStatus}`) {
            case 'pending->in_progress':
                ...
                if (!updatedTask.worktreePath || !updatedTask.branchName) {
                    throw new Error(`worktreePath and branchName must be set before transitioning to in_progress for task ${task.id}`);
                }
                break;
            case 'in_progress->in_review':
                ...
                if (!updatedTask.sessionId) {
                    ...
                    console.warn(`Warning: Task ${task.id} transitioning to in_review without sessionId`);
                }
                break;
            case 'in_progress->failed':
                // エラー情報が設定されていることを確認
                if (!updatedTask.error) {
                    console.warn(`⚠️  Warning: Task ${task.id} transitioning to failed without error message. Setting default error.`);
     ...
     */
    static canMoveToReady(task, allTasks) {
     ...
     */
    static validateTaskConsistency(tasks) {
        tasks.forEach((task) => {
            ...
            if (task.status === 'completed') {
                task.dependencies.forEach((depId) => {
                    ...
                    if (!dep) {
                        throw new Error(`Inconsistent state: task ${task.id} depends on non-existent task ${depId}`);
                    }
                    if (dep.status !== 'completed') {
                        throw new Error(`Inconsistent state: task ${task.id} is completed but dependency ${depId} is ${dep.status}`);
                    }
                });
            }
            ...
            if (task.status === 'in_progress') {
                if (!task.worktreePath || !task.branchName) {
                    console.warn(`Warning: Task ${task.id} is in_progress but missing worktreePath or branchName`);
                }
            }
        });
    }
     ...
     */
    static validateSprintCompletion(tasks, sprintId) {
        const sprintTasks = tasks.filter((t) => 'sprint' in t && t.sprint === sprintId);
