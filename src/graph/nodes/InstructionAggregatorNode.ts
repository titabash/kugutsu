 ...
 */
export async function instructionAggregatorNode(
  ...
  // 実行中タスクをカウント
  const tasksInProgress = sprintTasks.filter(
    task => task.instructionGenerating === true
  );

  // 生成済みタスクをカウント
  const tasksGenerated = sprintTasks.filter(
    task => task.instructionGenerated === true
  );
 ...
 */
export function instructionAggregatorRouter(
  state: ParallelDevStateType
): string | Send[] {
  if (!state.activeSprint?.id) {
  ...
  // 生成済みで開発準備完了のタスク
  const tasksReadyForDevelopment = sprintTasks.filter(
    task => task.instructionGenerated === true &&
            task.status === 'pending' &&
            !task.worktreePath
  );
  ...
  console.log(
    `📊 ルーティング判定: 未生成=${tasksNeedingInstruction.length}, ` +
  ...
  if (tasksNeedingInstruction.length > 0) {
    console.log(
      `🔄 instruction_generator_dispatch に送信 (未生成: ${tasksNeedingInstruction.length}件)`
    );
