...
export class DirectorAI extends BaseAI {
   ...
   */
  async createStoryMapping(userRequest: string, projectId: string): Promise<StoryMappingResult> {
   ...
   */
  private buildStoryMappingPrompt(userRequest: string): string {
    return `
...
`.trim();
   ...
   */
  private extractStoryMapping(response: string): StoryMapping {
    // JSONコードブロックを抽出
   ...
   */
  private generateStoryMappingMarkdown(storyMapping: StoryMapping): string {
   ...
   * 既存のストーリーマッピングを読み込み
   */
  async loadStoryMapping(projectId: string): Promise<StoryMapping | null> {
