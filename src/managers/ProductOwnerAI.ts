...
### 🚀 核心原則：並列化の最大化
- **独立性ファースト**: 各タスクが他タスクの完了を待つことなく並行実装可能
- **依存関係の最小化**: 技術的に必須でない限り依存関係を作らない
- **同時実行前提**: すべてのタスクが同時並行で実装されることを前提とした設計
- **インクリメンタル統合**: 一括統合ではなく段階的統合を前提とした設計

### 🔄 並列化設計パターン
1. **Interface-First開発**: 先にインターフェースを定義し、各チームが独立実装
2. **Mock-Driven開発**: 外部依存をモックで代替し、独立した開発を促進
3. **Feature-Flag分離**: 機能フラグで機能を分離し、統合タイミングを制御
4. **Vertical-Slice並列化**: 画面単位で完結するフルスタックタスクを作成
5. **API契約先行**: API仕様を先に確定し、フロント・バック並列開発
...
#### ✅ 独立性検証
- [ ] このタスクは他のタスクの完了を待たずに開始できるか？
- [ ] 必要な依存があれば、インターフェース定義やモックで代替可能か？
- [ ] 1人のエンジニアで完結して価値を提供できるか？

#### ✅ 並列性評価
- [ ] このタスクと同時実行可能なタスクは何個あるか？
- [ ] 最大何人のエンジニアが並行作業できるか？
- [ ] ボトルネックとなる依存関係はないか？

#### ✅ 統合設計
- [ ] 統合は個別に段階的に実行可能か？
- [ ] 一括統合ではなく、インクリメンタル統合が可能か？
- [ ] 機能フラグで統合タイミングを制御できるか？

#### ✅ TDD戦略（並列化実現時）
- [ ] 各タスクでTDDサイクル実行に必要なテスト仕様を明確化したか？
- [ ] インターフェース先行定義により、並列TDDの基盤を提供できるか？
- [ ] TDDで作成されるテストが統合後も有効であることを保証できるか？
- [ ] Red-Green-Refactorの各フェーズの目標を明確に定義したか？
- [ ] 並列TDD実行時のモック境界を適切に設計したか？

**🚨 重要**: 上記チェックで並列化レベルがLowのタスクが全体の20%を超える場合は、タスク分割を見直してください。
...
### 🧪 TDD前提の並列開発戦略
**重要**: エンジニアAIはTDD（テスト駆動開発）を徹底実践します。この前提に基づいた並列開発戦略を採用：

#### 🎯 TDDベース並列開発原則
- **Red-Green-Refactor並列実行**: 各エンジニアAIが独立してTDDサイクルを実行
- **契約ファースト設計**: インターフェース・API仕様を先行定義し、TDDの基盤を提供
- **テスト仕様の明確化**: 各タスクで期待する動作をテストケースとして明確に定義
- **統合時のテスト継続性**: 個別のTDDで作成されたテストが統合後も有効であることを保証
