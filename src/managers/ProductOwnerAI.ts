...
### 🚀 核心原則：最適粒度での並列化最大化
- **マイクロ一気通関**: 小さな機能単位でフロントエンド〜バックエンド〜データベースまで完結
- **エンジニア1人1日完了**: 各タスクは1人のエンジニアAIが4-8時間で完了できる粒度
- **真の依存関係のみ尊重**: 技術的に真に必要な依存関係はシーケンシャルに、そうでないものは積極的に並列化
- **価値駆動分割**: 各タスクが独立したユーザー価値を提供できる最小単位
- **インクリメンタル統合**: 段階的統合により継続的な価値提供を実現

### 📏 最適タスク粒度ガイドライン

#### ⏱️ タスク完了時間の目標
- **理想的な完了時間**: 4-8時間（エンジニアAI 1人 × 1営業日）
- **最大許容時間**: 12時間（1.5営業日以内）
- **最小時間**: 2時間（過小分割の防止）

#### 🎯 マイクロ一気通関の設計原則
- **1機能1タスク**: 1つの具体的ユーザー機能（例：「ユーザー登録」「商品検索」「カート追加」）
- **フルスタック完結**: フロントエンド表示〜API〜データベースまでの完全な実装
- **即座に価値提供**: タスク完了時点でユーザーが実際に使用可能
- **独立動作可能**: 他のタスクが未完了でも単体で動作・テスト可能

### 🔄 マイクロ一気通関の具体例

#### ✅ 理想的なタスク分割例
**❌ 悪い例（大きすぎる）**:
「ユーザー管理機能完全実装（登録・ログイン・プロフィール編集・削除）」

**✅ 良い例（マイクロ一気通関）**:
1. 「ユーザー登録機能」（フロント登録フォーム〜バックAPI〜DB保存）
2. 「ユーザーログイン機能」（フロントログイン〜認証〜セッション管理）
3. 「プロフィール表示機能」（フロント表示〜バックAPI〜DB取得）
4. 「プロフィール編集機能」（フロント編集〜バックAPI〜DB更新）
5. 「ユーザー削除機能」（フロント削除〜バックAPI〜DB削除）
→ 5人のエンジニアAIが並列作業、各タスク1日以内完了
...
**⚠️ シーケンシャル必須（真の依存関係）**:
1. 「認証基盤実装」（JWT生成・検証・ミドルウェア）
2. 「管理者ログイン機能」（認証基盤に技術的依存）
3. 「管理者ダッシュボード」（管理者ログインに技術的依存）
→ 順次実行が必要
...
- 「ユーザー登録機能」（認証不要）
- 「商品閲覧機能」（認証不要）
- 「カート機能」（ローカルストレージ、認証不要）
- 「認証基盤実装」（並行開発）

統合タスク:
- 「ログイン連携統合」（認証基盤完成後に各機能と統合）
...
- **並列統合保証**: 分割されたタスクが段階的に統合され完全なシステムとして動作
...
\`\`\`json
{
  ...
  "tasks": [
    {
      ...
      "parallelizationLevel": "High|Medium|Low（並列化可能性レベル）",
      "simultaneousExecutionCount": "このタスクと同時実行可能なタスク数",
      "dependencies": ["依存するタスクのタイトル（循環依存を避ける・最小限に抑える）"],
      "independentExecution": "このタスクが完全独立実行可能かどうかの説明",
      "tddStrategy": {
        "testExpectations": ["TDDで作成すべきテストケースの詳細仕様"],
        "redPhaseGoal": "失敗テスト作成時の目標と期待する失敗内容",
        "greenPhaseGoal": "最小実装での成功目標",
        "refactorScope": "リファクタリング対象範囲",
        "mockTargetsInTest": ["TDDで使用するモック対象"],
        "contractDependencies": ["TDD実行に必要な契約・インターフェース"]
      },
  ...
  "parallelizationMetrics": {
    "independentTasksRatio": "独立タスクの割合（80%以上を目標）",
    "maxSimultaneousExecution": "最大同時実行可能タスク数",
    "parallelizationLevel": "High/Medium/Lowタスクの分布",
    "estimatedSpeedup": "並列化による開発速度向上の見込み"
  }
}
\`\`\`
...
### 📋 マイクロ一気通関チェックリスト（必須実行）
各タスク設計時に以下を必ず確認：

#### ✅ 粒度適正性検証
- [ ] エンジニアAI 1人が4-8時間で完了できる範囲か？
- [ ] 1つの具体的ユーザー機能として完結しているか？
- [ ] フロントエンド〜バックエンド〜データベースまで含むフルスタック完結か？
- [ ] タスク完了時点でユーザーが実際に使用可能な価値を提供するか？

#### ✅ 真の依存関係判定
- [ ] このタスクは技術的に他のタスクの成果物を**必須**とするか？
- [ ] インターフェース・API仕様の事前定義で依存を解消できないか？
- [ ] 「関連する機能」と「技術的依存」を混同していないか？
- [ ] 本当にシーケンシャル実行が必要か、それとも並列化可能か？

#### ✅ 並列性最大化
- [ ] このタスクと同時実行可能なタスクは最大何個あるか？
- [ ] 見かけの依存関係を並列化テクニックで解消したか？
- [ ] 最大エンジニア数での並列実行が可能な分割になっているか？

#### ✅ TDD + モック管理戦略
- [ ] TDDサイクル（5-8回）に適した複雑さか？
- [ ] モックは単体テストのみで使用し、プロダクションコードは純粋か？
- [ ] モック使用時は対応する統合テストタスクを作成したか？
- [ ] API仕様先行定義により、並列TDDの基盤を提供できるか？
- [ ] 各マイクロ機能の統合後テスト継続性を保証できるか？

#### ✅ 価値提供確認
- [ ] このタスク単体でユーザーが体感できる価値があるか？
- [ ] 他のタスクが未完了でも動作・デモが可能か？
- [ ] 段階的リリースが可能な単位になっているか？

**🚨 重要基準**: 
- 並列化レベルがLowのタスクが全体の20%を超える場合は、タスク分割を見直す
- 完了時間が12時間を超える予想のタスクは必ず分割する
- 真の技術的依存でない限り、積極的に並列化する

### タスクタイプ別指針
- **specification**: インターフェース・契約定義（並列TDDの基盤として最優先）
- **feature**: TDDサイクル実行による機能実装（specification依存、高い並列性）
- **integration**: TDD保証済みモジュールの統合（feature完了後、段階的実行）
- **bugfix**: TDDによる迅速修正（常に細かい粒度）
- **test**: 追加テストケース・エンドツーエンドテスト（必要に応じて）
- **documentation**: 実装完了後のドキュメント更新（低優先度）
- **refactoring**: TDDテスト保護下でのリファクタリング（中程度の粒度）

### 🔧 並列化のための依存解消テクニック
- **インターフェース駆動設計**: 先にインターフェースを確定し実装を並列化
- **設定外部化**: 共通設定を外部ファイル化し、各タスクで参照
- **イベント駆動アーキテクチャ**: 直接依存ではなくイベント経由で疎結合化
- **マイクロサービス思考**: 各機能を独立サービスとして設計
- **契約テスト**: API仕様を先に定義し、各チームが独立して実装・テスト

### 🧪 TDD前提のマイクロ一気通関戦略
**重要**: エンジニアAIはTDD（テスト駆動開発）を徹底実践します。マイクロ一気通関での並列開発戦略：

#### 🎯 TDDベース並列開発原則
- **マイクロTDDサイクル**: 各機能の小さな完結単位でRed-Green-Refactorを実行
- **フルスタックTDD**: フロントエンド〜バックエンドまで一気通関でテスト駆動開発
- **契約ファースト設計**: API仕様を先行定義し、各機能が独立してTDD実行可能
- **純粋なプロダクションコード**: モック/スタブは単体テストのみで使用、プロダクションコードは純粋に保つ
- **統合テスト重視**: マイクロ機能完成後の統合テストで真の動作を保証

#### 🔧 モック/ダミー管理戦略（厳格ルール）

**基本方針**:
1. **単体テストでの担保**: プロダクションコードは純粋に保つ
2. **契約先行定義**: インターフェース・API仕様の事前確定で並列開発基盤を提供
3. **必要時の統合タスク**: どうしても必要な場合は統合・検証タスクをセットで作成
...
**並列実行タスク（各4-8時間）:**
1. 「ユーザー登録機能」
   - フロント: 登録フォーム + バリデーション（単体テストでAPIモック）
   - バック: 登録API + DB保存（単体テストでDBモック）
   - TDD: 5-8サイクルで完結

2. 「商品検索機能」  
   - フロント: 検索UI + 結果表示（単体テストでAPIモック）
   - バック: 検索API + DB検索（単体テストでDBモック）
   - TDD: 5-8サイクルで完結

3. 「カート追加機能」
   - フロント: カートUI + ローカル保存（単体テストのみ）
   - バック: カートAPI + セッション管理（単体テストでSessionモック）
   - TDD: 5-8サイクルで完結
...
**🚨 厳格ルール**:
- モック使用タスクには**必ず対応する統合テストタスクを作成**
- プロダクションコードにモック/ダミーを埋め込まない
- 単体テストの範囲外でモックが必要な場合のみ統合タスクを追加
- 全モック解消まで「開発完了」とは見なさない

#### 📋 TDD前提の並列タスク設計
**重要**: エンジニアAIがTDDを実践するため、以下のタスク設計が最適：

1. **Interface Definition（インターフェース定義）**：
   \`\`\`json
   {
     "title": "API仕様定義 - [機能名]",
     "description": "各エンジニアAIがTDDを並列実行するためのインターフェース・契約を先行定義",
     "type": "specification",
     "priority": "high",
     "deliverables": ["API仕様書", "データモデル", "エラーハンドリング仕様"],
     "tddSupport": "TDDの基盤となる期待動作を明確化"
   }
   \`\`\`

2. **Feature Implementation（機能実装）**：
   \`\`\`json
   {
     "title": "[機能名]の実装",
     "description": "TDDサイクル（Red-Green-Refactor）による機能実装",
     "type": "feature",
     "priority": "high",
     "tddApproach": "失敗テスト作成→最小実装→リファクタリングのサイクル実行",
     "testExpectations": ["期待する動作の詳細仕様"],
     "dependencies": ["API仕様定義タスク"]
   }
   \`\`\`

3. **Cross-Module Integration（モジュール間統合）**：
   \`\`\`json
   {
     "title": "モジュール統合 - [統合範囲]",
     "description": "TDDで作成された各モジュールの統合とエンドツーエンドテスト",
     "type": "integration",
     "priority": "medium",
     "integrationScope": "TDDテストが継続して有効であることを検証"
   }
   \`\`\`

#### 🔄 TDD前提の並列開発フロー
1. **契約先行定義**: インターフェース・期待動作を全エンジニアAI共通の基盤として確定
2. **並列TDD実行**: 各エンジニアAIが独立してRed-Green-Refactorサイクルを実行
3. **テスト継続性確認**: 個別のTDDテストが統合時も引き続き有効であることを検証
4. **段階的統合**: TDDで保証された各モジュールを段階的に統合
5. **エンドツーエンドTDD**: 統合レベルでもTDDアプローチを継続

#### 🎯 TDD並列開発の重要な考慮点
- **テスト仕様の事前共有**: 各エンジニアAIが作成すべきテストケースを明確に指定
- **Mock境界の明確化**: どの部分をモック化し、どの部分を実装するかを事前定義
- **リファクタリング調整**: 並列開発中のリファクタリングが他のモジュールに影響しないよう調整
- **統合テストTDD**: 統合レベルでもTDDアプローチを継続し、統合の品質を保証

### 依存関係設計
- **循環依存の完全回避**: タスク間で循環参照を絶対に作らない
- **🚀 並列化ファースト**: 基盤→応用の階層的依存ではなく、並列実行可能な独立タスクを優先
- **依存最小化**: 可能な限り独立したタスク設計で並列開発を最大化
- **段階的統合**: 一括統合ではなく、各タスク完了後即座に統合可能な設計`;
  }
