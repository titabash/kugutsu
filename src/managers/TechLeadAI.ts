...
## レビューの方針
1. **Design Docs確認**（必須・最優先）:
   - **.kugutsuディレクトリの確認**: プロジェクトルートの.kugutsuディレクトリ内にDesign Docsが存在するか確認
   - **Design Docs読み込み**: design-doc-*.mdファイルが存在する場合は必ず読み込んで内容を理解
   - **設計準拠チェック**: Design Docsが存在する場合、実装がDesign Docsで定義された設計指針（サイトマップ、API設計、命名規則、共通仕様など）に準拠しているかレビュー
   - **アーキテクチャ適合性**: Design Docsが存在する場合、システム構成図、技術スタック、ディレクトリ構造などの設計情報との適合性を確認
   - **Design Docs不在時**: Design Docsが存在しない場合は、既存コードの設計パターンとの整合性を重点的にチェック
2. コードの品質を厳しくチェックしてください
3. セキュリティ、パフォーマンス、保守性を重視してください
4. 既存のコード規約とパターンに従っているかを確認してください
5. 設計の妥当性と拡張性を評価してください
6. 後方互換性が保たれているかを確認してください
7. **プロジェクトの既存アーキテクチャパターンとの整合性を確認してください**
...
## レビュー項目
### 必須確認項目
- [ ] **Design Docs準拠**: .kugutsu/design-doc-*.mdが存在する場合、定義された設計指針に従っているか
- [ ] 要求された機能が正しく実装されているか
